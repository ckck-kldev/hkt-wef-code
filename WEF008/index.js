...
let pauseToggle = document.querySelector("#stop-game")
...
function setupGrid() {
    ...
    const canvas = createCanvas(windowWidth-50, windowHeight - 400);
...
function init() {
  for (let i = 0; i < columns; i++) {
    for (let j = 0; j < rows; j++) {
      if (randomInit == true) {
      currentBoard[i][j] = {life: random() > 0.9 ? 1 : 0, lifeStatus: 0, neighbor: 0} ; // one line if
      nextBoard[i][j] = {life: 0, lifeStatus: 0, neighbor: 0};
      } else {
      currentBoard[i][j] = {life: 0, lifeStatus: 0, neighbor: 0};
      nextBoard[i][j] = {life: 0, lifeStatus: 0, neighbor: 0};
      }
    }
  }
    ...
    for (let j = 0; j < rows; j++) {
      if (currentBoard[i][j].life == 1) {
        ...
        if (colorToggle.classList.contains('active')) {
          ...
          boxColor = 150
          if (currentBoard[i][j].lifeStatus == 1) {
            boxColor = color(32,178,170)
          }
...
function calcNeighbor(){
  for (let x = 0; x < columns; x++) {
    for (let y = 0; y < rows; y++) {
      currentBoard[x][y].neighbor = 0
    }
  }
      ...
      if(currentBoard[x][y].life == 1){
        for (let i of [-1, 0, 1]) {
          for (let j of [-1, 0, 1]) {
            ...
            currentBoard[(x + i + columns) % columns][(y + j + rows) % rows].neighbor++
          }
        }
      }
    }
...
function generate() {
    ...
    for (let x = 0; x < columns; x++) {
      for (let y = 0; y < rows; y++) {
        ...
        for (let i of [-1, 0, 1]) {
          for (let j of [-1, 0, 1]) {
            ...
            neighbors +=
              ...
              // currentBoard[(x + i + columns) % columns][(y + j + rows) % rows].neighbor = neighbors;
          }
        }
        currentBoard[x][y].neighbor = neighbors
        ...
        // Rules of Life
        if (currentBoard[x][y].life == 1 && neighbors < lonelinessDie) {
          // Die of Loneliness
          nextBoard[x][y].life = 0;
          nextBoard[x][y].lifeStatus = 0;
        } else if (currentBoard[x][y].life == 1 && neighbors > overpopulationDie) {
          ...
          // Stasis
          if (currentBoard[x][y].life == 1 && (currentBoard[x][y].neighbor == nextBoard[x][y].neighbor)) {
            nextBoard[x][y].lifeStatus = 1;
            // console.log(x,y,currentBoard[x][y], nextBoard[x][y])
          }
        ...
        if(currentBoard[x][y].life == 1){
          console.log({x,y,c:currentBoard[x][y],n:nextBoard[x][y]})
        }
      }
    }
